...
export function feedsGetPostThread(
  ...
  Result<
    operations.AppBskyFeedGetPostThreadResponse,
    | errors.AppBskyFeedGetPostThreadBadRequestError
    | errors.AppBskyFeedGetPostThreadAuthMissingError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.AppBskyFeedGetPostThreadRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/xrpc/app.bsky.feed.getPostThread")();

  const query = encodeFormQuery({
    "depth": payload.depth,
    "parentHeight": payload.parentHeight,
    "uri": payload.uri,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.AppBskyFeedGetPostThreadResponse$inboundSchema),
    M.jsonErr(
      400,
      errors.AppBskyFeedGetPostThreadBadRequestError$inboundSchema,
    ),
    M.jsonErr(
      401,
      errors.AppBskyFeedGetPostThreadAuthMissingError$inboundSchema,
    ),
